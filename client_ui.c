...
void choose_menu(int artnum) {
    ...
    timer = clock() - timer;
    req[rdsz] = '\0';
    if (strncmp(req, "CHECKSELF", 9) == 0) {
        int k;
        char * temp = malloc(5);
        k = sprintf(temp, "%d.0", artnum);
        temp[k] = '\0';
        for(k = 0; k < idx + 1; k++) {
            if (strcmp(articles[k].unique_id, temp) == 0) {
                memset(req, 0, rdsz);
                strcpy(req, articles[k].article);
                free(temp);
                break;
            }
        }
...
void reply_menu(int artnum) {
    ...
    articles[idx].unique_id = malloc(MAX_MSG);
    articles[idx].article = malloc(MAX_MSG);
    strcpy(articles[idx].unique_id, resp);
    strcpy(articles[idx].article, msg);
    idx++;
